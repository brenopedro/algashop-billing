...
import static com.algaworks.algashop.billing.infrastructure.resilience.SpringCircuitBreakerConfig.FASTPAY_PAYMENT_CB_ID;
...
public class ResilientFastpayPaymentAPIClient {
    ...
    public ResilientFastpayPaymentAPIClient(CircuitBreakerFactory<FrameworkRetryConfig,
    ...
    public FastpayPaymentModel capture(FastpayPaymentInput input) {
        log.info("Trying to capture payment for invoice {}", input.getReferenceCode());
    ...
    public FastpayPaymentModel findById(String paymentId) {
        log.info("Trying to load payment {}", paymentId);
